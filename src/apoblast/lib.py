...
class Library:
    ...
    def classify(self,expr):
        arr = []
        dic=sympy.expand(expr,).as_coefficients_dict(*self.model.coord,*self.model.field)
...
class Transformation:
    ...
    # jacovian_inverse
    # parameters
    def __init__(self,model,coord_replace,field_replace,parameter=(),jacovian_inverse=None):
        if not isinstance(model,Model):
            raise TypeError("model: Model")
        ...
        if not all(model.verify(v,with_field=False,parameter=parameter) for v in coord_replace) or len(coord_replace) != len(model.coord):
            #print(all(model.verify(v,with_field=False,parameter=parameter) for v in coord_replace),coord_replace,model.coord)
            raise TypeError("coord_replace: list<[new coord]>")
        if not all(model.verify(v,parameter=parameter) for v in field_replace) or len(field_replace) != len(model.field):
            raise TypeError("field_replace: list<[new field]>")
        ...
        self.coord_replace_dict=MappingProxyType({model.coord[i]:coord_replace[i] for i in range(0,len(model.coord))})
        self.field_replace_dict=MappingProxyType({model.field[i]:field_replace[i] for i in range(0,len(model.field))})
        ...
        self.jacovian_inverse_dict=MappingProxyType({model.coord[i]:self.jacovian_inverse.col(i) for i in range(0,len(model.coord))})
    ...
    def apply_internal(self,expr,cache):
        ...
        if isinstance(expr,sympy.core.expr.AtomicExpr):
            ...
            if axis in self.jacovian_inverse_dict:
                return sympy.Add(*(self.jacovian_inverse_dict[axis][i]*sympy.diff(transed_internal,self.model.coord[i]) for i in range(0,len(self.model.coord))))
            else:
    ...
    def __init__(self,library,constrainer):
        ...
        def trans(t):
            ...
            for i in range(0,len(constrainer.parameter_fix)):
                ...
                t=t.subs(constrainer.transformation.parameter[i],constrainer.parameter_fix[i])
            return t
...
class Solver:
    ...
    def condition_eigen(self,mat,filter=None):
        eigenmat=find_eigen(self.__kernel_matrix.pinv()*mat*self.__kernel_matrix,filter=filter)
    ...
    def shrink(self):
        ...
        lib=Library(self.__library.model,[self.__library.term[i] for i in nonnullrows])
        self.__library=lib
...
def collect_follower_raw(lhs,library,*trans,lhs_library=None,print_progress=False,nullspace_finder=find_nulls):
    if lhs_library==None:
        lhs_library=library
...
def collect_follower(lhs,library,*trans,lhs_library=None,print_progress=False,nullspace_finder=find_nulls):
    d=len(lhs)
    kernel_matrix,library=collect_follower_raw(lhs,library,*trans,lhs_library=lhs_library,print_progress=print_progress,nullspace_finder=nullspace_finder)
    R=len(library.term)
    li=[]
